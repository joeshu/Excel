...
1. 从“工作台”进入生成向导并上传 CSV/XLSX 基础数据。
2. 系统保存字段签名、行数、质量摘要和数据指纹，并返回候选工作流匹配结果。
3. 选择推荐工作流，或通过工作流向导创建当前数据源专属配置。
4. 配置通报标题、统计周期、发布单位、截止日期、落款和备注。
5. 生成成品 Excel，检查通报表、汇总、基础数据、工作流配置和质量报告。
6. 在结果中心查看批次状态、质量摘要、文件指纹、审计记录并下载单文件或批次 ZIP。
...
- 生成任务会输出包含 `通报表`、`基础数据`、`工作流配置` 和 `数据质量报告` 的成品 Excel
- 成品 Excel 还包含 `汇总` Sheet；批次 ZIP 还包含 `批次汇总.xlsx`
- 数据源支持字段签名、行数、质量摘要和 SHA-256 指纹
- 工作流支持按数据源字段结构匹配和向导创建
- 任务支持审计查询、输出哈希校验和本地角色权限矩阵
