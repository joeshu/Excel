...
type BatchSummary = { batch_id: string; task_count: number; pending_count: number; success_count: number; failed_count: number; status: string; notice_config: NoticeConfig; tasks: { id: number; data_source_id: number; status: string; output_path: boolean }[] };
...
function App() {
  ...
  const taskColumns = [{ title: "任务", render: (_: unknown, item: Task) => `#${item.id}` }, { title: "状态", render: (_: unknown, item: Task) => <Tag>{item.status}</Tag> }, { title: "计算引擎", dataIndex: "calculation_engine", render: (value: string) => value ?? "未执行" }, { title: "开始", dataIndex: "started_at" }, { title: "完成", dataIndex: "finished_at" }, { title: "操作", render: (_: unknown, item: Task) => <Space>{item.status === "success" && <><Button type="link" href={`/api/tasks/${item.id}/download`}>下载</Button><Button onClick={() => void showFinalPreview(item)}>成品预览</Button><Button onClick={() => void showTaskFormulaPreview(item)}>公式结果</Button></>}{["success", "failed"].includes(item.status) && <Button onClick={() => void retryTask(item)}>重试</Button>}</Space> }];
