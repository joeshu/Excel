...
function App() {
  ...
  const createDagWorkflow = async (item: Template) => { try { const response = await api.post<Workflow>("/workflows", { template_id: item.id, name: `${item.name} 模式B`, mode: "dag" }); setSelected(item); setWorkflow(response.data); setDagNodes([]); setDagEdges([]); setPage("dag"); await load(); } catch (reason) { showError(reason); } };
  ...
  const openDagWorkflow = (item: Workflow) => { setWorkflow(item); setDagNodes(item.node_json?.nodes ?? []); setDagEdges(item.node_json?.edges ?? []); setPage("dag"); };
  const addDagNode = (type: string) => { const id = `${type}-${Date.now()}`; const config = type === "formula" ? { field: "total", expression: "amount * 2" } : type === "condition" ? { field: "total", operator: "greater_than", value: 0 } : type === "write_template" ? { mapping: { A: "id", B: "amount", C: "total" } } : {}; setDagNodes((current) => [...current, { id, type: "default", position: { x: 80 + current.length * 40, y: 80 + current.length * 40 }, data: { label: type, nodeType: type, config } }]); setSelectedDagNodeId(id); };
  const updateDagNodeConfig = (value: string) => { if (!selectedDagNodeId) return; try { const config = JSON.parse(value); setDagNodes((current) => current.map((node) => node.id === selectedDagNodeId ? { ...node, data: { ...node.data, config } } : node)); } catch { /* Keep the editor responsive while JSON is incomplete. */ } };
  const saveDag = async () => { if (!workflow) return; try { const response = await api.put(`/workflows/${workflow.id}/dag`, { nodes: dagNodes.map((node) => ({ ...node, type: node.data.nodeType, data: { config: node.data.config ?? {}, label: node.data.label } })), edges: dagEdges }); setWorkflow(response.data.workflow); message.success("模式 B 流程已保存"); await load(); } catch (reason) { showError(reason); } };
  ...
  return <Layout className="shell">
    ...
    <Layout><Layout.Content className="content"><Typography.Title>Excel 工作流自动生成平台</Typography.Title>{error && <Alert closable message={error} type="error" onClose={() => setError(undefined)} />}
       ...
       {page === "dag" && <Card title="模式 B 流程设计器" extra={<Space><Button onClick={() => setPage("templates")}>返回</Button><Button type="primary" onClick={() => void saveDag()}>保存流程</Button><Button onClick={() => setPage("tasks")}>运行流程</Button></Space>}><Space wrap>{["data_source", "field_mapping", "formula", "condition", "write_template", "output_file"].map((type) => <Button key={type} onClick={() => addDagNode(type)}>添加 {type}</Button>)}</Space><div className="dag-canvas"><ReactFlow nodes={dagNodes} edges={dagEdges} onNodesChange={(changes) => setDagNodes((current) => current.map((node) => { const change = changes.find((entry) => entry.id === node.id); return change?.type === "position" && change.position ? { ...node, position: change.position } : node; }))} onEdgesChange={(changes) => setDagEdges((current) => current.filter((edge) => !changes.some((change) => change.id === edge.id && change.type === "remove")))} onConnect={(connection: Connection) => setDagEdges((current) => addEdge(connection, current))} onNodeClick={(_event, node) => setSelectedDagNodeId(node.id)} fitView><MiniMap /><Controls /><Background /></ReactFlow></div>{selectedDagNodeId && <Card size="small" title="节点配置"><Input.TextArea rows={6} value={JSON.stringify(dagNodes.find((node) => node.id === selectedDagNodeId)?.data.config ?? {}, null, 2)} onChange={(event) => updateDagNodeConfig(event.target.value)} /></Card>}</Card>}
