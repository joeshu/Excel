...
function App() {
  ...
  const showFormulaReport = async (item: Template) => { try { const response = await api.get(`/formulas/${item.id}/validate`); setFormulaReport(response.data); } catch (reason) { showError(reason); } };
  ...
  const templateColumns = [
    ...
    { title: "操作", render: (_: unknown, item: Template) => <Space><Button onClick={() => void showColumns(item)}>查看列</Button><Button onClick={() => void showPreview(item)}>预览</Button><Button onClick={() => void showFormulaReport(item)}>公式检查</Button><Button type="primary" onClick={() => void configureTemplate(item)}>配置模式 A</Button></Space> },
  ];
