...
function App() {
  ...
  const saveDag = async () => { if (!workflow) return; try { const persistedNodes = dagNodes.map((node) => { const config = node.data.config ?? {}; if (!["field_mapping", "write_template"].includes(node.data.nodeType)) return { ...node, type: node.data.nodeType, data: { config, label: node.data.label } }; const mapping = { ...(config.mapping ?? {}) }; columns.filter((column) => column.type !== "formula").forEach((column) => { const key = `${column.sheet}!${column.column}`; if (!mapping[key]) mapping[key] = suggestedField(column); }); return { ...node, type: node.data.nodeType, data: { config: { ...config, mapping }, label: node.data.label } }; }); const response = await api.put(`/workflows/${workflow.id}/dag`, { nodes: persistedNodes, edges: dagEdges }); setWorkflow(response.data.workflow); setDagNodes(persistedNodes); message.success("模式 B 流程已保存"); await load(); } catch (reason) { showError(reason); } };
