...
import { Alert, Button, Card, Form, Input, InputNumber, Layout, Menu, Modal, Progress, Select, Space, Steps, Table, Tabs, Tag, Typography, Upload, message } from "antd";
...
type Task = { id: number; workflow_id: number; data_source_id: number; status: string; error_log?: string; calculation_engine?: string; output_sha256?: string; started_at?: string; finished_at?: string };
...
function App() {
  ...
  const showWorkflowMatches = async () => { if (!dataSourceId) return message.warning("请先选择数据源"); try { const response = await api.get<{ matches: WorkflowMatch[] }>(`/data-sources/${dataSourceId}/workflow-matches`); setWorkflowMatches(response.data.matches); } catch (reason) { showError(reason); } };
  ...
  const templateColumns = [
    ...
    { title: "操作", render: (_: unknown, item: Template) => <Space><Button onClick={() => void showColumns(item)}>查看列</Button><Button onClick={() => void showPreview(item)}>预览</Button><Button onClick={() => void showFormulaReport(item)}>公式检查</Button>{item.is_example && tutorials.some((tutorialItem) => tutorialItem.scenario === item.name.replace("示例模板：", "")) && <Button onClick={() => showTutorial(item.name.replace("示例模板：", ""))}>教程</Button>}<Button onClick={() => void showVersions(item)}>版本</Button><Upload beforeUpload={(file) => uploadTemplate(file, item)} showUploadList={false}><Button>上传新版本</Button></Upload><Button onClick={() => void configureTemplate(item)}>配置模式 A</Button><Button type="primary" onClick={() => void createDagWorkflow(item)}>配置模式 B</Button></Space> },
  ];
  const workflowColumns = [{ title: "名称", render: (_: unknown, item: Workflow) => <Space>{item.name}{item.is_example && <Tag color="blue">示例</Tag>}</Space> }, { title: "模式", dataIndex: "mode" }, { title: "操作", render: (_: unknown, item: Workflow) => <Space>{item.mode === "dag" ? <Button onClick={() => openDagWorkflow(item)}>查看流程</Button> : <Button onClick={() => void openWorkflow(item)}>查看映射</Button>}{item.is_example && <Button onClick={() => void copyWorkflow(item)}>复制为我的工作流</Button>}{!item.is_example && <Button onClick={() => void copyWorkflow(item)}>复制</Button>}</Space> }];
   const taskColumns = [{ title: "任务", render: (_: unknown, item: Task) => `#${item.id}` }, { title: "状态", render: (_: unknown, item: Task) => <Tag>{item.status}</Tag> }, { title: "计算引擎", dataIndex: "calculation_engine", render: (value: string) => value ?? "未执行" }, { title: "文件指纹", render: (_: unknown, item: Task) => item.output_sha256 ? <Typography.Text copyable={{ text: item.output_sha256 }}>{item.output_sha256.slice(0, 12)}...</Typography.Text> : "未生成" }, { title: "开始", dataIndex: "started_at" }, { title: "完成", dataIndex: "finished_at" }, { title: "操作", render: (_: unknown, item: Task) => <Space><Button onClick={() => void showAudit(item)}>审计</Button>{item.status === "success" && <><Button type="link" href={`/api/tasks/${item.id}/download`}>下载</Button><Button onClick={() => void showFinalPreview(item)}>成品预览</Button><Button onClick={() => void showTaskFormulaPreview(item)}>公式结果</Button></>}{["success", "failed"].includes(item.status) && <Button onClick={() => void retryTask(item)}>重试</Button>}</Space> }];
