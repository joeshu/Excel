...
function App() {
  ...
  const load = async () => {
    const [templatesResponse, workflowsResponse, sourcesResponse, tasksResponse, tutorialsResponse] = await Promise.all([
      api.get<Template[]>("/templates"), api.get<Workflow[]>("/workflows"), api.get<DataSource[]>("/data-sources"), api.get<Task[]>("/tasks"), api.get<{ tutorials: Tutorial[] }>("/examples/tutorials"),
    ]);
